...
const Dropdown = memo(
  forwardRef((props: DropdownProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    React.useEffect(() => {
      ...
      if (trigger) {
        ...
        setMargin(
          props.direction === 'up'
            ? parseFloat(getComputedStyle(trigger).marginTop)
            : parseFloat(getComputedStyle(trigger).marginBottom),
        );
      ...
      if (dropdown) {
        dropdown.style.setProperty(
          '--mika-dropdown-position-X',
          props.position === 'center' || props.position === undefined ? '-50%' : '0',
        );
      }
      ...
      () => ({
        ...
        ...{
          ...
          left:
            props.position === 'left'
              ? '0'
              : props.position === 'center' || props.position === undefined
                ? '50%'
                : 'unset',
    ...
    return (
      <div className={`mika-dropdown-container${props.className ? ` ${props.className}` : ''}`} ref={ref}>
