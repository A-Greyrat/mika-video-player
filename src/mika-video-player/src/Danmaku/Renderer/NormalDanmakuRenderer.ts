...
export class NormalDanmakuRenderer implements IDanmakuRenderer {
  private getVelocity(width: number, speed: number): number {
  ...
  render(e: HTMLDivElement, danmaku: DanmakuAttr, extraData: DanmakuExtraData): void {
    ...
    const comparer = (i: Interval, danmaku: DanmakuAttr) => {
      ...
      const delayDistance = this.getVelocity(danmakuWidth, danmakuSpeed) * delay;
      return (
        delta * this.getVelocity(i.width, danmakuSpeed) + delayDistance <= containerWidth && // 前弹幕以及完全进入屏幕
        delta * this.getVelocity(danmakuWidth, danmakuSpeed) + delayDistance <= containerWidth // 在当前弹幕消失前，新弹幕不会追上前弹幕
      );
    };
    ...
    e.animate(
      [{ transform: `translate3d(${translateX}, 0, 0)` }, { transform: `translate3d(-${danmakuWidth}px, 0, 0)` }],
      {
