...
const QualityButton = memo(() => {
    ...
    const onClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        const quality = parseInt((e.target as HTMLDivElement).dataset.quality!);
    ...
    return (
        <Dropdown menu={<div className='mika-video-player-toolbar-function-Quality-dropdown-menu'>
            {qualityList.map((item, index) =>
                <div key={index}
                     className={'mika-video-player-toolbar-function-Quality-dropdown-item' + (activeIndex === index ? ' active' : '')}
                     data-quality={index} onClick={onClick}
                >{item}</div>)}</div>}
                  direction="up"
                  paddingTrigger={10}
        >
